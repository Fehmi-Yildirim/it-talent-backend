...
export class CandidateSkillsService {
  ...
  async create(userId: string, dto: CreateCandidateSkillDto) {
    ...
    const existingCandidateSkill = await this.prisma.candidateSkill.findUnique({
      where: {
        candidateId_skillId: {
          candidateId: candidate.id,
          skillId: skill.id,
    ...
    if (existingCandidateSkill) {
      throw new ConflictException('Candidate already has this skill');
    }
  ...
  async removeMine(userId: string, candidateSkillId: string) {
    const candidate = await this.prisma.candidate.findUnique({
      where: {
        userId,
      },
