...
describe('05 - Recruiter & Company (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const candidatePasswordHash = await argon2.hash(
            candidatePassword,
        );
        ...
        const candidateLoginResponse = await request(
            app.getHttpServer(),
        )
            .post('/api/v1/auth/login')
            .send({
        ...
        const secondRecruiterEmail =
            `e2e-recruiter-second-${Date.now()}@example.com`;
        ...
        const secondRecruiterLoginResponse = await request(
            app.getHttpServer(),
        )
            .post('/api/v1/auth/login')
            .send({
        ...
        secondRecruiterToken =
            secondRecruiterLoginResponse.body.accessToken;
    ...
    describe('company', () => {
        ...
        it('should allow the recruiter to create a company', async () => {
            const response = await request(app.getHttpServer())
                ...
                .send({
                    ...
                    description:
                        'Company created during E2E testing',
                })
                .expect(201);
        ...
        it('should update the recruiter company', async () => {
            const response = await request(app.getHttpServer())
                ...
                .send({
                    description:
                        'Updated E2E company description',
                })
                .expect(200);
    ...
    describe('ownership isolation', () => {
        it('should allow a second recruiter to create their own company', async () => {
            ...
            expect(secondCompanyId).toEqual(
                expect.any(String),
            );
        ...
        it('should update only the authenticated recruiter company', async () => {
            const response = await request(app.getHttpServer())
                ...
                .send({
                    description:
                        'Updated second recruiter company',
                })
                .expect(200);
            ...
            expect(firstCompany?.description).toBe(
                'Updated E2E company description',
            );
        });
