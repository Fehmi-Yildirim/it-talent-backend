...
describe('06 - Recruiter Authorization & Company Isolation (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const recruiterAPasswordHash = await argon2.hash(recruiterAPassword);
    ...
    const recruiterBPasswordHash = await argon2.hash(recruiterBPassword);
  ...
  it('should reject unauthenticated access to recruiter company', async () => {
    await request(app.getHttpServer()).get('/api/v1/companies/me').expect(401);
  });
